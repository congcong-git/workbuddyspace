...
type Tab = "posts" | "albums" | "new-post" | "edit-post" | "new-album" | "edit-album";
...
export default function AdminPage() {
  ...
  const PostEditor = ({ post, onSave, onCancel }: { post: PostData | null; onSave: () => void; onCancel: () => void }) => {
    ...
    const handleSave = async () => {
      if (!form.title || !slug) {
        showMessage("error", "标题和 slug 不能为空");
        return;
      }
      setSaving(true);
      const frontmatter = {
      ...
      const success = await saveFile(path, fileContent, `${post ? "更新" : "发布"}文章: ${form.title}`, sha);
      if (success) {
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onCancel} className="text-bark-400 hover:text-bark-600 transition-colors">
            取消
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-white dark:bg-bark-800 border border-warm-200 dark:border-bark-600 rounded-lg text-bark-700 dark:text-warm-100 focus:outline-none focus:ring-2 focus:ring-accent-400"
              placeholder="如：技术、随笔、阅读"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-bark-600 dark:text-bark-200 mb-1">标签（逗号分隔）</label>
            <input
              value={form.tags}
              onChange={(e) => setForm({ ...form, tags: e.target.value })}
              className="w-full px-3 py-2 bg-white dark:bg-bark-800 border border-warm-200 dark:border-bark-600 rounded-lg text-bark-700 dark:text-warm-100 focus:outline-none focus:ring-2 focus:ring-accent-400"
              placeholder="React, Next.js, 博客"
            />
          </div>
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="/images/posts/cover.jpg"
              />
        ...
        <div>
          ...
          <textarea
            ...
            rows={20}
            className="w-full px-3 py-2 bg-white dark:bg-bark-800 border border-warm-200 dark:border-bark-600 rounded-lg text-bark-700 dark:text-warm-100 focus:outline-none focus:ring-2 focus:ring-accent-400 font-mono text-sm leading-relaxed"
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onCancel} className="px-6 py-2.5 bg-warm-200 dark:bg-bark-700 text-bark-600 dark:text-bark-200 rounded-full font-medium hover:bg-warm-300 dark:hover:bg-bark-600 transition-colors">
            取消
          </button>
        </div>
      </div>
  ...
  const AlbumEditor = ({ album, onSave, onCancel }: { album: AlbumData | null; onSave: () => void; onCancel: () => void }) => {
    const [form, setForm] = useState({
      ...
      cover: album?.cover || "",
      photos: album?.photos || [] as { src: string; caption: string }[],
    });
    ...
    const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (url) {
          setForm((prev) => ({
            ...prev,
            photos: [...prev.photos, { src: url, caption: files[i].name.replace(/\.[^.]+$/, "") }],
          }));
        }
      }
      setUploading(false);
    ...
    const handleSave = async () => {
      if (!form.name || !slug) {
        showMessage("error", "相册名称和 slug 不能为空");
        return;
      }
      setSaving(true);
      const albumData = {
        ...
        date: form.date,
        cover: form.cover || (form.photos.length > 0 ? form.photos[0].src : ""),
        photos: form.photos,
      };
      const fileContent = yaml.dump(albumData, { lineWidth: -1 });
      const existingAlbum = album ? albums.find((a) => a.slug === slug) : null;
      const sha = existingAlbum?.sha;
      const path = `content/albums/${slug}.yaml`;
      const success = await saveFile(path, fileContent, `${album ? "更新" : "创建"}相册: ${form.name}`, sha);
      if (success) {
        showMessage("success", `相册${album ? "更新" : "创建"}成功！网站将自动部署`);
        onSave();
      } else {
        showMessage("error", "保存失败，请重试");
      }
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-bark-600 dark:text-bark-200 mb-1">相册名称</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="block text-sm font-medium text-bark-600 dark:text-bark-200 mb-1">描述</label>
            <input
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="block text-sm font-medium text-bark-600 dark:text-bark-200">
              照片 ({form.photos.length} 张)
            </label>
          ...
          {form.photos.length > 0 ? (
                ...
                <div key={index} className="group relative bg-white dark:bg-bark-800 rounded-lg overflow-hidden border border-warm-200 dark:border-bark-600">
                  ...
                  <div className="p-2">
                    <input
                      ...
                      className="w-full px-2 py-1 text-xs bg-warm-50 dark:bg-bark-700 border border-warm-200 dark:border-bark-600 rounded text-bark-600 dark:text-bark-200"
                      placeholder="照片说明"
                    />
                  </div>
                  <button
            ...
            <div className="text-center py-8 bg-white dark:bg-bark-800 rounded-lg border border-warm-200 dark:border-bark-600">
              ...
              <p className="text-bark-400 dark:text-bark-300 text-sm">还没有照片，点击上方按钮添加</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-bark-800 rounded-warm-lg p-6 border border-warm-200 dark:border-bark-700 shadow-sm">
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-3 py-2.5 bg-warm-50 dark:bg-bark-700 border border-warm-200 dark:border-bark-600 rounded-lg text-bark-700 dark:text-warm-100 focus:outline-none focus:ring-2 focus:ring-accent-400"
              placeholder="请输入用户名"
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <button onClick={loadData} className="px-4 py-2 text-sm bg-warm-200 dark:bg-bark-700 text-bark-600 dark:text-bark-200 rounded-full hover:bg-warm-300 dark:hover:bg-bark-600 transition-colors">
            🔄 刷新
          </button>
          <a href="/" className="px-4 py-2 text-sm bg-warm-200 dark:bg-bark-700 text-bark-600 dark:text-bark-200 rounded-full hover:bg-warm-300 dark:hover:bg-bark-600 transition-colors">
            🌐 查看网站
          </a>
          <button onClick={handleLogout} className="px-4 py-2 text-sm bg-warm-200 dark:bg-bark-700 text-red-500 rounded-full hover:bg-warm-300 dark:hover:bg-bark-600 transition-colors">
            退出
          </button>
        </div>
      </div>
      ...
      {message && (
        <div className={`mb-6 px-4 py-3 rounded-warm-lg text-sm font-medium ${message.type === "success" ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300" : "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300"}`}>
          {message.text}
        </div>
      )}
      ...
      {activeTab === "posts" && !loading && (
        <div>
          <button onClick={() => { setEditingPost(null); setActiveTab("new-post"); }} className="mb-6 px-5 py-2.5 bg-accent-400 text-white rounded-full font-medium hover:bg-accent-500 transition-colors shadow-md">
            ✍️ 发布新文章
          </button>
          {posts.length > 0 ? (
              ...
              {posts.map((post) => (
                <div key={post.slug} className="flex items-center justify-between p-4 bg-white dark:bg-bark-800 rounded-warm-lg border border-warm-200 dark:border-bark-700 hover:border-accent-300 dark:hover:border-accent-500 transition-all">
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    <button onClick={async () => { if (confirm(`确定删除「${post.title}」吗？`)) { const success = await deleteFile(`content/posts/${post.slug}.md`, `删除文章: ${post.title}`, post.sha); if (success) { showMessage("success", "文章已删除"); loadData(); } else { showMessage("error", "删除失败"); } } }} className="px-3 py-1.5 text-xs bg-red-100 dark:bg-red-900/30 text-red-500 rounded-full hover:bg-red-200 transition-colors">删除</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-12 bg-white dark:bg-bark-800 rounded-warm-lg border border-warm-200 dark:border-bark-700">
              ...
              <p className="text-bark-400 dark:text-bark-300">暂无文章</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "albums" && !loading && (
        <div>
          <button onClick={() => { setEditingAlbum(null); setActiveTab("new-album"); }} className="mb-6 px-5 py-2.5 bg-accent-400 text-white rounded-full font-medium hover:bg-accent-500 transition-colors shadow-md">
            📷 创建新相册
          </button>
          {albums.length > 0 ? (
            <div className="space-y-3">
              {albums.map((album) => (
                <div key={album.slug} className="flex items-center justify-between p-4 bg-white dark:bg-bark-800 rounded-warm-lg border border-warm-200 dark:border-bark-700 hover:border-accent-300 dark:hover:border-accent-500 transition-all">
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    <button onClick={async () => { if (confirm(`确定删除「${album.name}」吗？`)) { const success = await deleteFile(`content/albums/${album.slug}.yaml`, `删除相册: ${album.name}`, album.sha); if (success) { showMessage("success", "相册已删除"); loadData(); } else { showMessage("error", "删除失败"); } } }} className="px-3 py-1.5 text-xs bg-red-100 dark:bg-red-900/30 text-red-500 rounded-full hover:bg-red-200 transition-colors">删除</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-12 bg-white dark:bg-bark-800 rounded-warm-lg border border-warm-200 dark:border-bark-700">
              <span className="text-4xl block mb-3">📷</span>
              <p className="text-bark-400 dark:text-bark-300">暂无相册</p>
            </div>
          )}
        </div>
      )}
